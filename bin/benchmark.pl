...
my @intervals = ( map { int ($_ * 1000) }(.1, .5, 1..100) );
my $last = 0;
my $next = 10;
my $i = shift @ARGV || 0;
$|++;
...
while(1){
    ...
    # create objects
    for(1..$how_many){ ### Creating $how_many objects ($last total) [%]
    ...
    print {$datafile} "$last, ";
    my $start = time;
    for(1..50000){ ### Running 50000 searches on $last objects [%]
